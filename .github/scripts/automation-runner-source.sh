...
if [ ! -d "$runner_dir/.git" ]; then
  rm -rf "$runner_dir"
  git init "$runner_dir"
  git -C "$runner_dir" remote add origin "$repo_url"
fi

git -C "$runner_dir" fetch --depth 1 origin "$runner_ref"
git -C "$runner_dir" checkout --detach FETCH_HEAD

if [ ! -x "$runner_dir/node_modules/.bin/tsx" ]; then
  npm ci --prefix "$runner_dir" --no-audit --no-fund
fi
